...
KAPP_APP ?= mapping-externalsecrets
...
manifests: ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	...
	cat hack/boilerplate.yaml.txt > config/mapping-externalsecrets.yaml
	$(KUSTOMIZE) build config/default >> config/mapping-externalsecrets.yaml
...
fmt: ## Run go fmt against code.
	$(GOIMPORTS) --local github.com/servicebinding/mapping-externalsecrets -w .
...
deploy: manifests ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	$(KAPP) deploy -a $(KAPP_APP) -n $(KAPP_APP_NAMESPACE) -c \
		-f config/kapp \
		-f <($(KO) resolve --platform $(KO_PLATFORMS) -f config/mapping-externalsecrets.yaml)
...
.PHONY: deploy-external-secrets
deploy-external-secrets: ## Deploy external-secrets to the K8s cluster specified in ~/.kube/config.
	$(KAPP) deploy -a external-secrets -n $(KAPP_APP_NAMESPACE) --wait-timeout 5m -c -f https://github.com/external-secrets/external-secrets/releases/download/v0.8.1/external-secrets.yaml

.PHONY: undeploy-external-secrets
undeploy-external-secrets: ## Undeploy external-secrets from the K8s cluster specified in ~/.kube/config.
	$(KAPP) delete -a external-secrets -n $(KAPP_APP_NAMESPACE)
